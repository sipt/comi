...
import type { FormattedTextRenderOptions } from "vs/base/browser/formattedTextRenderer";
...
export function renderMarkdown(
  ...
  if (options.codeBlockRenderer) {
    renderer.code = (code, lang) => {
      ...
      raceCancellation(Promise.all([value, withInnerHTML]), cts.token)
        .then((values) => {
  ...
  dompurify.addHook("uponSanitizeAttribute", (element, e) => {
    if (e.attrName === "style" || e.attrName === "class") {
      if (element.tagName === "SPAN") {
        if (e.attrName === "style") {
          e.keepAttr =
            /^(color\:#[0-9a-fA-F]+;)?(background-color\:#[0-9a-fA-F]+;)?$/.test(
              e.attrValue
            );
          return;
...
function getSanitizerOptions(options: { readonly isTrusted?: boolean }): {
