...
export class MultiplexLayoutController implements ILayoutController {
  get isLayoutEnabled(): boolean {
    return this.layoutControllers.every((l) => l.isLayoutEnabled);
  }
...
class LeafNode implements ISplitView<ILayoutContext>, IDisposable {
  ...
  constructor(readonly view: IView, readonly orientation: Orientation, readonly layoutController: ILayoutController, orthogonalSize: number, size: number = 0) {
    this._orthogonalSize = orthogonalSize;
    this._size = size;
...
export class GridView implements IDisposable {
  ...
  constructor(options: IGridViewOptions = {}) {
    ...
    this.layoutController = new MultiplexLayoutController([this.firstLayoutController, ...(options.layoutController ? [options.layoutController] : [])]);
