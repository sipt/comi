...
import type { IContextMenuProvider } from "vs/base/browser/contextmenu";
import {
...
import type { IFocusTracker } from "vs/base/browser/dom";
...
export class ButtonWithDropdown extends Disposable implements IButton {
  ...
  constructor(container: HTMLElement, options: IButtonWithDropdownOptions) {
    ...
    this.dropdownButton = this._register(
      new Button(this.element, { ...options, title: false, supportIcons: true })
    );
...
export class ButtonWithDescription
  extends Disposable
  implements IButtonWithDescription
{
  private _element: HTMLElement;
