...
export class Dialog extends Disposable {
    ...
    return new Promise<IDialogResult>((resolve) => {
      ...
      buttonMap.forEach((entry, index) => {
        ...
        const button = this.options.buttonDetails
          ? this._register(buttonBar.addButtonWithDescription({ title: true, secondary: !primary }))
          : this._register(buttonBar.addButton({ title: true, secondary: !primary }));
      ...
      this.iconElement.classList.remove(
        ...dialogErrorIcon.classNamesArray,
        ...dialogWarningIcon.classNamesArray,
        ...dialogInfoIcon.classNamesArray,
        ...Codicon.loading.classNamesArray,
      ...
      if (!this.options.disableCloseAction) {
        ...
        const action = this._register(
          new Action("dialog.close", nls.localize("dialogClose", "Close Dialog"), dialogCloseIcon.classNames, true, async () => {
            resolve({
              button: this.options.cancelId || 0,
              checkboxChecked: this.checkbox ? this.checkbox.checked : undefined,
            });
  ...
  private applyStyles() {
    if (this.styles) {
      ...
      if (fgColor && bgColor) {
        const messageDetailColor = fgColor.transparent(0.9);
