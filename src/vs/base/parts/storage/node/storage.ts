...
import type { IStorageDatabase, IStorageItemsChangeEvent, IUpdateRequest } from "vs/base/parts/storage/common/storage";
      ...
      import("@vscode/sqlite3").then((sqlite3) => {
        const connection: IDatabaseConnection = {
          db: new (this.logger.isTracing ? sqlite3.verbose().Database : sqlite3.Database)(path, (error) => {
            ...
            return this.exec(connection, ["PRAGMA user_version = 1;", "CREATE TABLE IF NOT EXISTS ItemTable (key TEXT UNIQUE ON CONFLICT REPLACE, value BLOB)"].join("")).then(
              () => {
