...
import { type ExtensionKind, IEnvironmentService, type IExtensionHostDebugParams } from "vs/platform/environment/common/environment";
import type { IPath } from "vs/platform/window/common/window";
import type { IWorkbenchEnvironmentService } from "vs/workbench/services/environment/common/environmentService";
import type { IWorkbenchConstructionOptions } from "vs/workbench/browser/web.api";
import type { IProductService } from "vs/platform/product/common/productService";
...
export class BrowserWorkbenchEnvironmentService implements IBrowserWorkbenchEnvironmentService {
  ...
  @memoize
  get logsPath(): string {
    return this.logsHome.path;
  }

  @memoize
  get logLevel(): string | undefined {
    return this.payload?.get("logLevel") || (this.options.developmentOptions?.logLevel !== undefined ? LogLevelToString(this.options.developmentOptions?.logLevel) : undefined);
  }

  @memoize
  get logFile(): URI {
    return joinPath(this.logsHome, "window.log");
  }
  ...
  get settingsResource(): URI {
    return joinPath(this.userRoamingDataHome, "settings.json");
  }
  ...
  get snippetsHome(): URI {
    return joinPath(this.userRoamingDataHome, "snippets");
  }
  ...
  get globalStorageHome(): URI {
    return URI.joinPath(this.userRoamingDataHome, "globalStorage");
  }

  @memoize
  get workspaceStorageHome(): URI {
    return URI.joinPath(this.userRoamingDataHome, "workspaceStorage");
  }
  ...
  get userDataSyncLogResource(): URI {
    return joinPath(this.logsHome, "userDataSync.log");
  }
  ...
  get keybindingsResource(): URI {
    return joinPath(this.userRoamingDataHome, "keybindings.json");
  }
  ...
  get telemetryLogResource(): URI {
    return joinPath(this.logsHome, "telemetry.log");
  }
  ...
  constructor(private readonly workspaceId: string, private readonly logsHome: URI, readonly options: IWorkbenchConstructionOptions, private readonly productService: IProductService) {
