...
import { Extensions as ColorRegistryExtensions, type IColorRegistry, type ColorIdentifier, editorBackground, editorForeground } from "vs/platform/theme/common/colorRegistry";
import { type ITokenStyle, getThemeTypeSelector } from "vs/platform/theme/common/themeService";
...
import { type MatcherWithPriority, type Matcher, createMatchers } from "vs/workbench/services/themes/common/textMateScopeMatcher";
import type { IExtensionResourceLoaderService } from "vs/workbench/services/extensionResourceLoader/common/extensionResourceLoader";
import { CharCode } from "vs/base/common/charCode";
import { StorageScope, IStorageService, StorageTarget } from "vs/platform/storage/common/storage";
import type { ThemeConfiguration } from "vs/workbench/services/themes/common/themeConfiguration";
    ...
    if (!this.textMateThemingRules) {
      ...
      function addRule(rule: ITextMateThemingRule) {
        if (rule.scope && rule.settings) {
          ...
          result.push({
            scope: rule.scope,
            settings: { foreground: normalizeColor(rule.settings.foreground), background: normalizeColor(rule.settings.background), fontStyle: rule.settings.fontStyle },
          });
        }
      }
...
async function _loadColorTheme(
  ...
  themeLocation: URI,
  result: { textMateRules: ITextMateThemingRule[]; colors: IColorMap; semanticTokenRules: SemanticTokenRule[]; semanticHighlighting: boolean }
): Promise<any> {
