...
import { AbstractExtensionsInitializer, type IExtensionsInitializerPreviewResult } from "vs/platform/userDataSync/common/extensionsSync";
...
import {
  ...
  type IUserDataSyncStoreClient,
  IUserDataSyncStoreManagementService,
  SyncResource,
} from "vs/platform/userDataSync/common/userDataSync";
import { type AuthenticationSessionInfo, getCurrentAuthenticationSessionInfo } from "vs/workbench/services/authentication/browser/authenticationService";
import { getSyncAreaLabel } from "vs/workbench/services/userDataSync/common/userDataSync";
import { type IWorkbenchContribution, type IWorkbenchContributionsRegistry, Extensions } from "vs/workbench/common/contributions";
import { Registry } from "vs/platform/registry/common/platform";
import { LifecyclePhase } from "vs/workbench/services/lifecycle/common/lifecycle";
import { isWeb } from "vs/base/common/platform";
import { Barrier, Promises } from "vs/base/common/async";
import { IExtensionGalleryService, IExtensionManagementService, IGlobalExtensionEnablementService, type ILocalExtension } from "vs/platform/extensionManagement/common/extensionManagement";
  ...
  private async initializeUserDataSyncStore(authenticationSession: AuthenticationSessionInfo): Promise<void> {
    ...
    try {
      const userDataSyncStoreClient = disposables.add(
        new UserDataSyncStoreClient(userDataSyncStore.url, this.productService, this.requestService, this.logService, this.environmentService, this.fileService, this.storageService)
      );
      userDataSyncStoreClient.setAuthToken(authenticationSession.accessToken, authenticationSession.providerId);
      ...
      if (this.globalStateUserData) {
        const userDataSyncStoreType = new UserDataSyncStoreTypeSynchronizer(userDataSyncStoreClient, this.storageService, this.environmentService, this.fileService, this.logService).getSyncStoreType(
  ...
  private createSyncResourceInitializer(syncResource: SyncResource): IUserDataInitializer {
    switch (syncResource) {
      case SyncResource.Settings:
        return new SettingsInitializer(this.fileService, this.environmentService, this.logService, this.uriIdentityService);
      case SyncResource.Keybindings:
        return new KeybindingsInitializer(this.fileService, this.environmentService, this.logService, this.uriIdentityService);
      case SyncResource.Tasks:
        return new TasksInitializer(this.fileService, this.environmentService, this.logService, this.uriIdentityService);
      case SyncResource.Snippets:
        return new SnippetsInitializer(this.fileService, this.environmentService, this.logService, this.uriIdentityService);
      case SyncResource.GlobalState:
        return new GlobalStateInitializer(this.storageService, this.fileService, this.environmentService, this.logService, this.uriIdentityService);
    }
    ...
    for (const galleryExtension of galleryExtensions) {
      try {
        ...
        const local = await this.extensionManagementService.installFromGallery(
          galleryExtension,
          { isMachineScoped: false, donotIncludePackAndDependencies: true, installPreReleaseVersion: extensionToSync.preRelease } /* set isMachineScoped to prevent install and sync dialog in web */
        );
