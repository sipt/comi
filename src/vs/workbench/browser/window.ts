...
export class BrowserWindow extends Disposable {
  ...
  private onWillShutdown(): void {
    ...
    Event.toPromise(Event.any(Event.once(new DomEmitter(document.body, EventType.KEY_DOWN, true).event), Event.once(new DomEmitter(document.body, EventType.MOUSE_DOWN, true).event))).then(
      async () => {
        ...
        // state.
        const res = await this.dialogService.show(Severity.Error, localize("shutdownError", "An unexpected error occurred that requires a reload of this page."), [localize("reload", "Reload")], {
          ...
          if (isSafari) {
            ...
            if (!opened) {
              const showResult = await this.dialogService.show(
                ...
                [localize("open", "Open"), localize("learnMore", "Learn More"), localize("cancel", "Cancel")],
                {
                  cancelId: 2,
                  detail: href,
                }
