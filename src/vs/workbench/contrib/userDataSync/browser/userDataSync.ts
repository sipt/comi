...
import { registerEditorContribution, type ServicesAccessor } from "vs/editor/browser/editorExtensions";
import type { IEditorContribution } from "vs/editor/common/editorCommon";
import type { ITextModel } from "vs/editor/common/model";
import { IModelService } from "vs/editor/common/services/model";
import { ILanguageService } from "vs/editor/common/languages/language";
import { type ITextModelContentProvider, ITextModelService } from "vs/editor/common/services/resolverService";
import { localize } from "vs/nls";
import { MenuId, MenuRegistry, registerAction2, Action2 } from "vs/platform/actions/common/actions";
import { CommandsRegistry, ICommandService } from "vs/platform/commands/common/commands";
import { IConfigurationService } from "vs/platform/configuration/common/configuration";
import { ContextKeyExpr, type IContextKey, IContextKeyService, RawContextKey } from "vs/platform/contextkey/common/contextkey";
import { IDialogService } from "vs/platform/dialogs/common/dialogs";
import { IInstantiationService } from "vs/platform/instantiation/common/instantiation";
import { INotificationService, Severity } from "vs/platform/notification/common/notification";
import { IQuickInputService, type IQuickPickItem, type IQuickPickSeparator } from "vs/platform/quickinput/common/quickInput";
...
import { IActivityService, type IBadge, NumberBadge, ProgressBadge } from "vs/workbench/services/activity/common/activity";
...
import { ViewContainerLocation, type IViewContainersRegistry, Extensions, type ViewContainer } from "vs/workbench/common/views";
...
const resolveKeybindingsConflictsCommand = {
  id: "workbench.userDataSync.actions.resolveKeybindingsConflicts",
  title: localize("showKeybindingsConflicts", "{0}: Show Keybindings Conflicts", SYNC_TITLE),
};
...
export class UserDataSyncWorkbenchContribution extends Disposable implements IWorkbenchContribution {
    ...
    switch (error.code) {
      ...
      case UserDataSyncErrorCode.ServiceChanged:
        this.notificationService.notify({
          severity: Severity.Info,
          message:
            this.userDataSyncStoreManagementService.userDataSyncStore?.type === "insiders"
  ...
  private handleTooLargeError(resource: SyncResource, message: string, error: UserDataSyncError): void {
    ...
    this.notificationService.notify({
      ...
      actions: {
        primary: [
          new Action("open sync file", localize("open file", "Open {0} File", getSyncAreaLabel(resource)), undefined, true, () =>
  ...
  private handleInvalidContentError(source: SyncResource): void {
    ...
    const handle = this.notificationService.notify({
      ...
      actions: {
        primary: [
          new Action("open sync file", localize("open file", "Open {0} File", errorArea), undefined, true, () =>
  ...
  private async updateGlobalActivityBadge(): Promise<void> {
    ...
    if (this.userDataSyncService.conflicts.length && this.userDataSyncEnablementService.isEnabled()) {
      badge = new NumberBadge(
        this.userDataSyncService.conflicts.reduce((result, [, conflicts]) => {
          return result + conflicts.length;
        }, 0),
        () => localize("has conflicts", "{0}: Conflicts Detected", SYNC_TITLE)
      );
  ...
  private async updateAccountBadge(): Promise<void> {
    ...
    if (
      ...
      this.userDataSyncWorkbenchService.accountStatus === AccountStatus.Unavailable
    ) {
  ...
  private async turnOnSyncAfterInitialization(): Promise<void> {
    ...
    const result = await this.dialogService.show(
      ...
      [localize("turn on settings sync", "Turn On Settings Sync"), localize("cancel", "Cancel")],
      {
        cancelId: 1,
        custom: {
          markdownDetails: [
            ...
            {
              markdown: new MarkdownString(
                `${localize({ key: "change later", comment: ["Context here is that user can change (turn on/off) settings sync later."] }, "You can always change this later.")} [${localize(
                  "learn more",
                  "Learn More"
    ...
    try {
      ...
      if (e instanceof UserDataSyncError) {
        ...
        this.notificationService.error(
          localize("turn on failed with user data sync error", "Error while turning on Settings Sync. Please check [logs]({0}) for more details.", `command:${SHOW_SYNC_LOG_COMMAND_ID}`)
        );
      } else {
  ...
  private async turnOff(): Promise<void> {
    const result = await this.dialogService.confirm({
      ...
      checkbox:
        this.userDataSyncWorkbenchService.accountStatus === AccountStatus.Available
          ? {
  ...
  private registerTurnOnSyncAction(): void {
    const turnOnSyncWhenContext = ContextKeyExpr.and(
      CONTEXT_SYNC_STATE.notEqualsTo(SyncStatus.Uninitialized),
      CONTEXT_SYNC_ENABLEMENT.toNegated(),
      CONTEXT_ACCOUNT_STATE.notEqualsTo(AccountStatus.Uninitialized),
  ...
  private registerTurnOnSyncAfterInitializationAction(): void {
    ...
    const when = ContextKeyExpr.and(
      ...
      CONTEXT_TURNING_ON_STATE.negate(),
      CONTEXT_SYNC_AFTER_INITIALIZATION
    );
    this._register(
      registerAction2(
        class AskToTurnOnSync extends Action2 {
          constructor() {
            super({
              id,
  ...
  private registerShowSnippetsConflictsAction(): void {
    ...
    this._snippetsConflictsActionsDisposable.add(
      MenuRegistry.appendMenuItem(MenuId.GlobalActivity, {
        group: "5_sync",
        command: {
            ...
            return new Promise<void>((c, e) => {
              ...
              if (that.userDataSyncEnablementService.canToggleEnablement()) {
                ...
                items.push({
                  ...
                  label: turnOffSyncCommand.title,
                  description: account ? `${account.accountName} (${that.authenticationService.getLabel(account.authenticationProviderId)})` : undefined,
                });
              }
              quickPick.items = items;
              disposables.add(
...
class AcceptChangesContribution extends Disposable implements IEditorContribution {
  ...
  private createAcceptChangesWidgetRenderer(): void {
    if (!this.acceptChangesButton) {
      ...
      this._register(
        this.acceptChangesButton.onClick(async () => {
          const model = this.editor.getModel();
          if (model) {
            this.telemetryService.publicLog2<{ source: string; action: string }, SyncConflictsClassification>("sync/handleConflicts", {
              source: syncResource,
              action: isRemote ? "acceptRemote" : "acceptLocal",
            });
            const syncAreaLabel = getSyncAreaLabel(syncResource);
            const result = await this.dialogService.confirm({
              type: "info",
              title: isRemote ? localize("Sync accept remote", "{0}: {1}", SYNC_TITLE, acceptRemoteLabel) : localize("Sync accept merges", "{0}: {1}", SYNC_TITLE, acceptMergesLabel),
              message: isRemote
  ...
  private getSyncResourceConflicts(resource: URI): [SyncResource, IResourcePreview[]] | undefined {
    return this.userDataSyncService.conflicts.filter(([, conflicts]) =>
      conflicts.some(({ previewResource, remoteResource }) => isEqual(previewResource, resource) || isEqual(remoteResource, resource))
    )[0];
  }
