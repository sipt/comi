...
import { IQuickInputService, type IQuickPickItem } from "vs/platform/quickinput/common/quickInput";
...
export class ShowAudioCueHelp extends Action2 {
  ...
  override async run(accessor: ServicesAccessor): Promise<void> {
    ...
    const quickPick = quickPickService.pick<IQuickPickItem & { audioCue: AudioCue }>(items, {
      activeItem: items[0],
      onDidFocus: (item) => {
