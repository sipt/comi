...
import { type IDisposable, dispose, Disposable, DisposableStore, combinedDisposable, MutableDisposable } from "vs/base/common/lifecycle";
import { Event } from "vs/base/common/event";
import { VIEW_PANE_ID, ISCMService, type ISCMRepository, ISCMViewService } from "vs/workbench/contrib/scm/common/scm";
import { IActivityService, NumberBadge } from "vs/workbench/services/activity/common/activity";
import type { IWorkbenchContribution } from "vs/workbench/common/contributions";
import { type IContextKey, IContextKeyService } from "vs/platform/contextkey/common/contextkey";
...
export class SCMActiveResourceContextKeyController implements IWorkbenchContribution {
  ...
  private updateContextKey(): void {
    ...
    if (activeResource?.scheme === Schemas.file || activeResource?.scheme === Schemas.vscodeRemote) {
      const activeResourceRepository = this.scmService.repositories.find((r) => r.provider.rootUri && this.uriIdentityService.extUri.isEqualOrParent(activeResource, r.provider.rootUri));
      ...
      for (const resourceGroup of activeResourceRepository?.provider.groups.elements ?? []) {
        if (resourceGroup.elements.some((scmResource) => this.uriIdentityService.extUri.isEqual(activeResource, scmResource.sourceUri))) {
