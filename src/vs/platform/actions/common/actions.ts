...
import { Action, type IAction, Separator, SubmenuAction } from "vs/base/common/actions";
import { CSSIcon } from "vs/base/common/codicons";
import { Emitter, Event } from "vs/base/common/event";
import { Iterable } from "vs/base/common/iterator";
import { DisposableStore, type IDisposable, toDisposable } from "vs/base/common/lifecycle";
import { LinkedList } from "vs/base/common/linkedList";
import type { ICommandAction, ICommandActionTitle, Icon, ILocalizedString } from "vs/platform/action/common/action";
import { CommandsRegistry, type ICommandHandlerDescription, ICommandService } from "vs/platform/commands/common/commands";
import { ContextKeyExpr, type ContextKeyExpression, IContextKeyService } from "vs/platform/contextkey/common/contextkey";
import { SyncDescriptor, type SyncDescriptor0 } from "vs/platform/instantiation/common/descriptors";
import { type BrandedService, createDecorator, type IConstructorSignature, type ServicesAccessor } from "vs/platform/instantiation/common/instantiation";
import { type IKeybindingRule, type IKeybindings, KeybindingsRegistry } from "vs/platform/keybinding/common/keybindingsRegistry";
...
export class SubmenuItemAction extends SubmenuAction {
  constructor(readonly item: ISubmenuItem, private readonly _menuService: IMenuService, private readonly _contextKeyService: IContextKeyService, private readonly _options?: IMenuActionOptions) {
...
export class MenuItemAction implements IAction {
  ...
  constructor(
    ...
    this.id = item.id;
    this.label =
      options?.renderShortTitle && item.shortTitle ? (typeof item.shortTitle === "string" ? item.shortTitle : item.shortTitle.value) : typeof item.title === "string" ? item.title : item.title.value;
...
export class SyncActionDescriptor {
  ...
  public static create<Services extends BrandedService[]>(
    ctor: { new (id: string, label: string, ...services: Services): Action },
    id: string,
    label: string | undefined,
    keybindings?: IKeybindings,
    keybindingContext?: ContextKeyExpression,
  ...
  private constructor(
    ctor: IConstructorSignature<Action, [string, string | undefined]>,
    id: string,
    label: string | undefined,
    keybindings?: IKeybindings,
    keybindingContext?: ContextKeyExpression,
