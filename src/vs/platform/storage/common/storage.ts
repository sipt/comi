...
import { InMemoryStorageDatabase, type IStorage, Storage } from "vs/base/parts/storage/common/storage";
...
export abstract class AbstractStorageService extends Disposable implements IStorageService {
  ...
  async flush(reason = WillSaveStateReason.NONE): Promise<void> {
    ...
    switch (reason) {
      ...
      case WillSaveStateReason.NONE:
        await Promises.settled([globalStorage?.whenFlushed() ?? Promise.resolve(), workspaceStorage?.whenFlushed() ?? Promise.resolve()]);
        break;

      // Shutdown: we want to flush as soon as possible
      // and not hit any delays that might be there
      case WillSaveStateReason.SHUTDOWN:
        await Promises.settled([globalStorage?.flush(0) ?? Promise.resolve(), workspaceStorage?.flush(0) ?? Promise.resolve()]);
        break;
    }
  }
  ...
  async logStorage(): Promise<void> {
    ...
    return logStorage(globalItems, workspaceItems, this.getLogDetails(StorageScope.GLOBAL) ?? "", this.getLogDetails(StorageScope.WORKSPACE) ?? "");
  }
