...
import { AbstractInitializer, AbstractSynchroniser, type IAcceptResult, type IMergeResult, type IResourcePreview, isSyncData } from "vs/platform/userDataSync/common/abstractSynchronizer";
...
import type { UserDataSyncStoreClient } from "vs/platform/userDataSync/common/userDataSyncStoreService";
...
export class GlobalStateSynchroniser extends AbstractSynchroniser implements IUserDataSynchroniser {
  ...
  constructor(
    ...
    this._register(
      Event.any(
        ...
        Event.filter(storageService.onDidChangeValue, (e) =>
          e.scope === StorageScope.GLOBAL && e.target !== undefined ? e.target === StorageTarget.USER : storageService.keys(StorageScope.GLOBAL, StorageTarget.USER).includes(e.key)
        ),
  ...
  private getStorageKeys(lastSyncGlobalState: IGlobalState | null): StorageKeys {
    ...
    const unregistered = lastSyncGlobalState?.storage
      ? Object.keys(lastSyncGlobalState.storage).filter((key) => !key.startsWith(argvStoragePrefx) && !registered.includes(key) && this.storageService.get(key, StorageScope.GLOBAL) !== undefined)
      : [];
