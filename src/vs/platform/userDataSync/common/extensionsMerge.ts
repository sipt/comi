...
import type { ISyncExtension, ISyncExtensionWithVersion } from "vs/platform/userDataSync/common/userDataSync";
    ...
    const merge = (key: string, updatedInRemote: boolean): ISyncExtensionWithVersion | undefined => {
      ...
      if (localExtension) {
        ...
        return {
          ...
          preRelease: isUndefined(mergedExtension.preRelease) /* from older client*/ ? localExtension.preRelease : localExtension.installed ? mergedExtension.preRelease : remoteExtension.preRelease,
        };
      }
      return undefined;
    };
...
function compare(
  ...
  { checkInstalledProperty, checkVersionProperty }: { checkInstalledProperty: boolean; checkVersionProperty: boolean } = { checkInstalledProperty: false, checkVersionProperty: false }
  ...
  const added = toKeys
    .filter((key) => fromKeys.indexOf(key) === -1)
    .reduce((r, key) => {
      r.add(key);
      return r;
    }, new Set<string>());
  const removed = fromKeys
    .filter((key) => toKeys.indexOf(key) === -1)
    .reduce((r, key) => {
      r.add(key);
      return r;
    }, new Set<string>());
