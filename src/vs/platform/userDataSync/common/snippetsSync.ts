...
import { FileOperationError, FileOperationResult, type IFileContent, IFileService, type IFileStat } from "vs/platform/files/common/files";
import { IStorageService } from "vs/platform/storage/common/storage";
import { ITelemetryService } from "vs/platform/telemetry/common/telemetry";
import { IUriIdentityService } from "vs/platform/uriIdentity/common/uriIdentity";
import { AbstractInitializer, AbstractSynchroniser, type IAcceptResult, type IFileResourcePreview, type IMergeResult } from "vs/platform/userDataSync/common/abstractSynchronizer";
import { areSame, type IMergeResult as ISnippetsMergeResult, merge } from "vs/platform/userDataSync/common/snippetsMerge";
import {
  Change,
...
export class SnippetsSynchroniser extends AbstractSynchroniser implements IUserDataSynchroniser {
    ...
    if (content) {
      ...
      if (syncData) {
        ...
        for (const snippet of Object.keys(snippets)) {
          ...
          result.push({
            resource,
            comparableResource: exists ? comparableResource : this.extUri.joinPath(this.syncPreviewFolder, snippet).with({ scheme: USER_DATA_SYNC_SCHEME, authority: "local" }),
          });
        }
        return result;
      }
  ...
  override async resolveContent(uri: URI): Promise<string | null> {
    if (
      this.extUri.isEqualOrParent(uri, this.syncPreviewFolder.with({ scheme: USER_DATA_SYNC_SCHEME, authority: "remote" })) ||
