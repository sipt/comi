...
import { type IExtUri, isEqualOrParent, joinPath } from "vs/base/common/resources";
...
import { allSettings, ConfigurationScope, Extensions as ConfigurationExtensions, type IConfigurationRegistry } from "vs/platform/configuration/common/configurationRegistry";
import type { IEnvironmentService } from "vs/platform/environment/common/environment";
import { EXTENSION_IDENTIFIER_PATTERN, type IExtensionIdentifier } from "vs/platform/extensionManagement/common/extensionManagement";
...
export function getDefaultIgnoredSettings(): string[] {
  ...
  const machineSettings = Object.keys(allSettings).filter(
    (setting) => allSettings[setting].scope === ConfigurationScope.MACHINE || allSettings[setting].scope === ConfigurationScope.MACHINE_OVERRIDABLE
  );
...
export function isAuthenticationProvider(thing: any): thing is IAuthenticationProvider {
  return thing && isObject(thing) && isString(thing.id) && isArray(thing.scopes);
}
