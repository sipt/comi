...
import { type JSONVisitor, parse, visit } from "vs/base/common/json";
import { applyEdits, setProperty, withFormatting } from "vs/base/common/jsonEdit";
import { type Edit, type FormattingOptions, getEOL } from "vs/base/common/jsonFormatter";
import * as objects from "vs/base/common/objects";
import type { IConfigurationService } from "vs/platform/configuration/common/configuration";
import * as contentUtil from "vs/platform/userDataSync/common/content";
import { getDisallowedIgnoredSettings, type IConflictSetting } from "vs/platform/userDataSync/common/userDataSync";
...
function getEditToInsertAtLocation(content: string, key: string, value: any, location: InsertLocation, tree: INode[], formattingOptions: FormattingOptions): Edit[] {
  ...
  if (location.insertAfter) {
    ...
    const content =
      (tree[location.index - 1] && !tree[location.index - 1].setting /* previous node is comment */ ? eol : "") + newProperty + (findNextSettingNode(location.index, tree) ? "," : "") + eol;
