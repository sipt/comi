...
import { type CancelablePromise, createCancelablePromise, ThrottledDelayer } from "vs/base/common/async";
import { VSBuffer } from "vs/base/common/buffer";
import { CancellationToken } from "vs/base/common/cancellation";
import type { IStringDictionary } from "vs/base/common/collections";
import { Emitter, Event } from "vs/base/common/event";
import { parse, type ParseError } from "vs/base/common/json";
import type { FormattingOptions } from "vs/base/common/jsonFormatter";
...
import type { IHeaders } from "vs/base/parts/request/common/request";
import { localize } from "vs/nls";
import { IConfigurationService } from "vs/platform/configuration/common/configuration";
import { IEnvironmentService } from "vs/platform/environment/common/environment";
import { FileChangesEvent, FileOperationError, FileOperationResult, type IFileContent, IFileService } from "vs/platform/files/common/files";
...
import {
  ...
  type ISyncResourcePreview as IBaseSyncResourcePreview,
  type IUserData,
...
export abstract class AbstractSynchroniser extends Disposable implements IUserDataSynchroniser {
  ...
  get conflicts(): IBaseResourcePreview[] {
  ...
  private async performSync(remoteUserData: IRemoteUserData, lastSyncUserData: IRemoteUserData | null, apply: boolean, userDataSyncConfiguration: IUserDataSyncConfiguration): Promise<SyncStatus> {
    if (remoteUserData.syncData && remoteUserData.syncData.version > this.version) {
      ...
      throw new UserDataSyncError(
        localize(
          { key: "incompatible", comment: ["This is an error while syncing a resource that its local version is not compatible with its remote version."] },
  ...
  async merge(resource: URI): Promise<ISyncResourcePreview | null> {
    await this.updateSyncResourcePreview(resource, async (resourcePreview) => {
      ...
      const acceptResult: IAcceptResult | undefined =
        mergeResult && !mergeResult.hasConflicts ? await this.getAcceptResult(resourcePreview, resourcePreview.previewResource, undefined, CancellationToken.None) : undefined;
    ...
    try {
      ...
      if (this.hasSyncResourceStateVersionChanged) {
        this.logService.info(
          `${this.syncResourceLogLabel}: Reset last sync state because last sync state version ${parsed.version} is not compatible with current sync state version ${resourceSyncStateVersion}.`
        );
