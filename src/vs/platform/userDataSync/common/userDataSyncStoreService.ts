...
import type { ConfigurationSyncStore } from "vs/base/common/product";
import { joinPath, relativePath } from "vs/base/common/resources";
import { isArray, isObject, isString } from "vs/base/common/types";
import { URI } from "vs/base/common/uri";
import { generateUuid } from "vs/base/common/uuid";
import type { IHeaders, IRequestContext, IRequestOptions } from "vs/base/parts/request/common/request";
...
export abstract class AbstractUserDataSyncStoreManagementService extends Disposable implements IUserDataSyncStoreManagementService {
  ...
  constructor(
    ...
    this._register(
      Event.filter(
        ...
        (e) => e.key === SYNC_SERVICE_URL_TYPE && e.scope === StorageScope.GLOBAL && this.userDataSyncStoreType !== this.userDataSyncStore?.type
      )(() => this.updateUserDataSyncStore())
    );
  }
  ...
  protected toUserDataSyncStore(productStore: (ConfigurationSyncStore & { web?: ConfigurationSyncStore }) | undefined, configuredStore?: ConfigurationSyncStore): UserDataSyncStore | undefined {
    ...
    if (
      ...
      const url = configuredStore?.url || (type === "insiders" ? syncStore.insidersUrl : type === "stable" ? syncStore.stableUrl : syncStore.url);
      return {
        url: URI.parse(url),
        type,
        defaultType,
...
export class UserDataSyncStoreManagementService extends AbstractUserDataSyncStoreManagementService implements IUserDataSyncStoreManagementService {
  ...
  constructor(@IProductService productService: IProductService, @IConfigurationService configurationService: IConfigurationService, @IStorageService storageService: IStorageService) {
    ...
    if (this._donotMakeRequestsUntil?.getTime() !== donotMakeRequestsUntil?.getTime()) {
      ...
      if (this._donotMakeRequestsUntil) {
        ...
        this.resetDonotMakeRequestsUntilPromise = createCancelablePromise((token) =>
          timeout(this._donotMakeRequestsUntil!.getTime() - Date.now(), token).then(() => this.setDonotMakeRequestsUntil(undefined))
        );
