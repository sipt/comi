...
import { IInstantiationService, type ServiceIdentifier, type ServicesAccessor, _util } from "vs/platform/instantiation/common/instantiation";
...
export class Trace {
  ...
  stop() {
    ...
    let lines = [`${this.type === TraceType.Creation ? "CREATE" : "CALL"} ${this.name}`, `${printChild(1, this)}`, `DONE, took ${dur.toFixed(2)}ms (grand total ${Trace._totals.toFixed(2)}ms)`];
